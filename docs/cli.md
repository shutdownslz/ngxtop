...
ngxtop [options] query <query> ...        # 执行自定义 SQL（表名为 log，表结构按数据自动推断）
```
...
| `query <sql>...` | 执行自定义 SQL（表名为 `log`，表结构由首条记录的字段自动推断） | `ngxtop query 'select status, count(1) as c from log group by status'` |
...
- **硬性要求**：每行必须是一个**扁平 JSON 对象**（`{...}`）；否则整行被跳过。
- **没有任何字段是硬性必填**：缺 `status`/`bytes_sent`/`request_time` 时按 `0` 处理，缺 URI 时 `request_path` 为
  `None`，程序不会报错，只是相应统计为 0 或空。
- **要让默认报表有意义的最小字段集**：
  - `status` —— 默认 Summary 的 2xx/3xx/4xx/5xx 统计与 `status_type` 依赖它；
  - 一个 URI 别名（`request_uri`/`req_uri`/`uri`）—— 默认按 `request_path` 分组依赖它。
  - 再补 `bytes_sent`(别名) 和 `request_time`(别名) 即可把默认 Summary 的所有列填满。
- **key 命名约束**：所有 JSON 键都会保留，但凡是要用于 `-g`/`-i`/`-o`/`-w`/`query` 的键，**应为合法标识符**
  （`snake_case`，字母/数字/下划线、不以数字开头）。含点号、连字符、空格的键虽被保留，却无法在 `-i` 的 Python
  表达式或 SQL 中直接引用。
- **`query` 子命令的建表行为**：`query` 用**第一条记录的键**动态建表，第一行没有的键在后续行会被丢弃，因此各行键
  集合应保持一致（或首行含键的超集）。其余命令（默认/`print`/`top`/`avg`/`sum`）用显式字段列，不受此影响。
...
- **`query` / `-i` / `-p` 使用 `eval` 或原始 SQL**：只应传入你自己可信的表达式与语句，不要拼接不可信内容。
...
# 自定义 SQL（表名 log，列由数据自动推断）
ngxtop -f json -l access.json.log --no-follow \
  query 'select status_type, count(1) as cnt, round(avg(request_time),3) as avg_s from log group by status_type order by cnt desc'
```
